...
** History:
**
**   - 2018-11-20  0.1   Lux First version.
**
******************************************************************************/
...
typedef struct
{
    union
    {
        __IO uint32_t PA00_SEL;						//0x00	端口 PA00 功能配置寄存器 PA00_SEL
        stc_gpio_pa00_sel_field_t PA00_SEL_f;
    };
    union
    {
        __IO uint32_t PA01_SEL;						//0x04	端口 PA01 功能配置寄存器（PA01_SEL）
        stc_gpio_pa01_sel_field_t PA01_SEL_f;
    };
    union
    {
        __IO uint32_t PA02_SEL;						//0x08 	PA02 功能配置寄存器（PA02_SEL）
        stc_gpio_pa02_sel_field_t PA02_SEL_f;
    };
    union
    {
        __IO uint32_t PA03_SEL;						//0x0C	端口 PA03 功能配置寄存器（PA03_SEL）
        stc_gpio_pa03_sel_field_t PA03_SEL_f;
    };
    union
    {
        __IO uint32_t PA04_SEL;						//0x10	端口 PA04 功能配置寄存器（PA04_SEL）
        stc_gpio_pa04_sel_field_t PA04_SEL_f;
    };
    union
    {
        __IO uint32_t PA05_SEL;						//0x14	端口 PA05 功能配置寄存器（PA05_SEL）
    ...
    union
    {
        __IO uint32_t PA15_SEL;					//0x3C	端口 PA15 功能配置寄存器（PA15_SEL）
    ...
    union
    {
        __IO uint32_t PADIR;					//0x100	端口 PA 输入输出配置寄存器（PADIR）		0/1:输出/输入
        stc_gpio_padir_field_t PADIR_f;
    };
    union
    {
        __IO uint32_t PAIN;						//0x104	端口 PA 输入值寄存器（PAIN）
        stc_gpio_pain_field_t PAIN_f;
    };
    union
    {
        __IO uint32_t PAOUT;					//0x108	端口 PA 输出值配置寄存器（PAOUT）
        stc_gpio_paout_field_t PAOUT_f;
    };
    union
    {
        __IO uint32_t PAADS;					//0x10C   端口 PA 数模配置寄存器（PAADS）  对应PA15-PA00 :  0/1:数字/模拟端口
        stc_gpio_paads_field_t PAADS_f;
    };
    union
    {
        __IO uint32_t PABSET;					//0x110	端口 PA 位置位寄存器（PABSET） （对应PA15-PA00  	0/1:保持/置位
        stc_gpio_pabset_field_t PABSET_f;
    };
    union
    {
        __IO uint32_t PABCLR;					//0x114	端口 PA 位清零寄存器（PABCLR）	（对应PA15-PA00	0/1:保持/保持
        stc_gpio_pabclr_field_t PABCLR_f;
    };
    union
    {
        __IO uint32_t PABSETCLR;				//0x118	端口 PA 位置位清零寄存器（PABSETCLR）
        stc_gpio_pabsetclr_field_t PABSETCLR_f;
    };
    union
    {
        __IO uint32_t PADR;					//0x11C	端口 PA 驱动能力配置寄存器（PADR）	（对应PA15-PA00  0/1:高/低 驱动能力
        stc_gpio_padr_field_t PADR_f;
    };
    union
    {
        __IO uint32_t PAPU;					//0x120	端口 PA 上拉使能配置寄存器（PAPU）	（对应PA15-PA00）0/1:禁止/使能上拉
        stc_gpio_papu_field_t PAPU_f;
    };
    union
    {
        __IO uint32_t PAPD;					//0x124	端口 PA 下拉使能配置寄存器（PAPD）	（对应PA15-PA00）0/1:禁止/使能下拉
        stc_gpio_papd_field_t PAPD_f;
    };
    uint8_t RESERVED66[4];
    union
    {
        __IO uint32_t PAOD;					//0x12C	端口 PA 开漏输出配置寄存器（PAOD）	（对应PA15-PA00）0/1:推完/开漏输出
        stc_gpio_paod_field_t PAOD_f;
    };
    union
    {
        __IO uint32_t PAHIE;					//0x130	端口 PA 高电平中断使能配置寄存器（PAHIE）（对应PA15-PA00）0/1:禁止/使能 高电平中断
        stc_gpio_pahie_field_t PAHIE_f;
    };
    union
    {
        __IO uint32_t PALIE;					//0x134 	端口 PA 低电平中断使能配置寄存器（PALIE）（对应PA15-PA00）0/1:禁止/使能 低电平中断
        stc_gpio_palie_field_t PALIE_f;
    };
    union
    {
        __IO uint32_t PARIE;					//0x138	端口 PA 上升沿中断使能配置寄存器（PARIE）对应PA15-PA00）0/1:禁止/使能上升沿中断
        stc_gpio_parie_field_t PARIE_f;
    };
    union
    {
        __IO uint32_t PAFIE;					//0x13C	端口 PA 下降沿中断使能配置寄存器（PAFIE）对应PA15-PA00）0/1:禁止/使能下降沿中断
        stc_gpio_pafie_field_t PAFIE_f;
    };
    union
    {
        __IO uint32_t PBDIR;                                   //0x140	端口 PB 输入输出配置寄存器（PBDIR）			0/1:输出/输入
        stc_gpio_pbdir_field_t PBDIR_f;
    };
    union
    {
        __IO uint32_t PBIN;                                    //0x144	端口 PB 输入值寄存器（PBIN）
        stc_gpio_pbin_field_t PBIN_f;
    };
    union
    {
        __IO uint32_t PBOUT;                                   //0x148	端口 PB 输出值配置寄存器（PBOUT）
        stc_gpio_pbout_field_t PBOUT_f;
    };
    union
    {
        __IO uint32_t PBADS;                                   //0x14C   端口 PB 数模配置寄存器（PBADS）  对应PB15-PB00 :  0/1:数字/模拟端口
        stc_gpio_pbads_field_t PBADS_f;
    };
    union
    {
        __IO uint32_t PBBSET;                                  //0x150	端口 PB位置位寄存器（PBBSET） （对应PB15-PB00  	0/1:保持/置位
        stc_gpio_pbbset_field_t PBBSET_f;
    };
    union
    {
        __IO uint32_t PBBCLR;                                  //0x154	端口 PB 位清零寄存器（PBBCLR）	（对应PB15-PB00	0/1:保持/保持
        stc_gpio_pbbclr_field_t PBBCLR_f;
    };
    union
    {
        __IO uint32_t PBBSETCLR;                               //0x158	端口 PB 位置位清零寄存器（PBBSETCLR）
        stc_gpio_pbbsetclr_field_t PBBSETCLR_f;
    };
    union
    {
        __IO uint32_t PBDR;                                    //0x15C	端口 PB 驱动能力配置寄存器（PBDR）	（对应PB15-PB00  0/1:高/低 驱动能力
        stc_gpio_pbdr_field_t PBDR_f;
    };
    union
    {
        __IO uint32_t PBPU;                                    //0x160	端口 PB 上拉使能配置寄存器（PBPU）	（对应PB15-PB00）0/1:禁止/使能上拉
        stc_gpio_pbpu_field_t PBPU_f;
    };
    union
    {
        __IO uint32_t PBPD;                                    //0x164	端口 PB 下拉使能配置寄存器（PBPD）	（对应PB15-PB00）0/1:禁止/使能下拉
        stc_gpio_pbpd_field_t PBPD_f;
    };
    uint8_t RESERVED81[4];
    union
    {
        __IO uint32_t PBOD;                                    //0x16C	端口 PB 开漏输出配置寄存器（PBOD）	（对应PB15-PB00）0/1:推完/开漏输出
        stc_gpio_pbod_field_t PBOD_f;
    };
    union
    {
        __IO uint32_t PBHIE;                                   //0x170	端口 PB 高电平中断使能配置寄存器（PBHIE）（对应PB15-PB00）0/1:禁止/使能 高电平中断
        stc_gpio_pbhie_field_t PBHIE_f;
    };
    union
    {
        __IO uint32_t PBLIE;                                   //0x174 	端口 PB 低电平中断使能配置寄存器（PBLIE）（对应PB15-PB00）0/1:禁止/使能 低电平中断
        stc_gpio_pblie_field_t PBLIE_f;
    };
    union
    {
        __IO uint32_t PBRIE;                                   //0x178	端口 PB 上升沿中断使能配置寄存器（PBRIE）对应PB15-PB00）0/1:禁止/使能上升沿中断
        stc_gpio_pbrie_field_t PBRIE_f;
    };
    union
    {
        __IO uint32_t PBFIE;                                   //0x17C	端口 PB 下降沿中断使能配置寄存器（PBFIE）对应PB15-PB00）0/1:禁止/使能下降沿中断
        stc_gpio_pbfie_field_t PBFIE_f;
    };
    union
    {
        __IO uint32_t PCDIR;                                  //0x180	端口 PC 输入输出配置寄存器（PCDIR）	0/1:输出/输入
        stc_gpio_pcdir_field_t PCDIR_f;
    };
    union
    {
        __IO uint32_t PCIN;                                   	//0x184	端口 PC 输入值寄存器（PCIN）
        stc_gpio_pcin_field_t PCIN_f;
    };
    union
    {
        __IO uint32_t PCOUT;                                  //0x188	端口 PC 输出值配置寄存器（PCOUT）
        stc_gpio_pcout_field_t PCOUT_f;
    };
    union
    {
        __IO uint32_t PCADS;                                  //0x18C   端口 PC 数模配置寄存器（PCADS）  对应PC15-PC00 :  0/1:数字/模拟端口
        stc_gpio_pcads_field_t PCADS_f;
    };
    union
    {
        __IO uint32_t PCBSET;                                 //0x190	端口 PC 位置位寄存器（PCBSET） （对应PC15-PC00  	0/1:保持/置位
        stc_gpio_pcbset_field_t PCBSET_f;
    };
    union
    {
        __IO uint32_t PCBCLR;                                 //0x194	端口 PC 位清零寄存器（PCBCLR）	（对应PC15-PC00	0/1:保持/保持
        stc_gpio_pcbclr_field_t PCBCLR_f;
    };
    union
    {
        __IO uint32_t PCBSETCLR;                              //0x198	端口 PC 位置位清零寄存器（PCBSETCLR）
        stc_gpio_pcbsetclr_field_t PCBSETCLR_f;
    };
    union
    {
        __IO uint32_t PCDR;                                   //0x19C	端口 PC 驱动能力配置寄存器（PCDR）	（对应PC15-PC00  0/1:高/低 驱动能力
        stc_gpio_pcdr_field_t PCDR_f;
    };
    union
    {
        __IO uint32_t PCPU;                                   //0x1A0	端口 PC 上拉使能配置寄存器（PCPU）	（对应PC15-PC00）0/1:禁止/使能上拉
        stc_gpio_pcpu_field_t PCPU_f;
    };
    union
    {
        __IO uint32_t PCPD;                                   //0x1A4	端口 PC下拉使能配置寄存器（PCPD）	（对应PC15-PC00）0/1:禁止/使能下拉
        stc_gpio_pcpd_field_t PCPD_f;
    };
    uint8_t RESERVED96[4];
    union
    {
        __IO uint32_t PCOD;                                   //0x1AC	端口 PC 开漏输出配置寄存器（PCOD）	（对应PC15-PC00）0/1:推完/开漏输出
        stc_gpio_pcod_field_t PCOD_f;
    };
    union
    {
        __IO uint32_t PCHIE;                                  //0x1B0	端口 PC 高电平中断使能配置寄存器（PCHIE）（对应PC15-PC00）0/1:禁止/使能 高电平中断
        stc_gpio_pchie_field_t PCHIE_f;
    };
    union
    {
        __IO uint32_t PCLIE;                                  //0x1B4 	端口 PC 低电平中断使能配置寄存器（PCLIE）（对应PC15-PC00）0/1:禁止/使能 低电平中断
        stc_gpio_pclie_field_t PCLIE_f;
    };
    union
    {
        __IO uint32_t PCRIE;                                  //0x1B8		端口 PC 上升沿中断使能配置寄存器（PCRIE）对应PC15-PC00）0/1:禁止/使能上升沿中断
        stc_gpio_pcrie_field_t PCRIE_f;
    };
    union
    {
        __IO uint32_t PCFIE;                                  //0x1BC		端口 PC 下降沿中断使能配置寄存器（PCFIE）对应PC15-PC00）0/1:禁止/使能下降沿中断
        stc_gpio_pcfie_field_t PCFIE_f;
    };
    union
    {
        __IO uint32_t PDDIR;                                  //0x1C0	端口 PD 输入输出配置寄存器（PDDIR）	0/1:输出/输入
        stc_gpio_pddir_field_t PDDIR_f;
    };
    union
    {
        __IO uint32_t PDIN;                                   	//0x1C4	端口 PD 输入值寄存器（PDIN）
        stc_gpio_pdin_field_t PDIN_f;
    };
    union
    {
        __IO uint32_t PDOUT;                                  //0x1C8	端口 PD 输出值配置寄存器（PDOUT）
        stc_gpio_pdout_field_t PDOUT_f;
    };
    union
    {
        __IO uint32_t PDADS;                                  //0x1CC   端口 PD 数模配置寄存器（PDADS）  对应PD15-PD00 :  0/1:数字/模拟端口
        stc_gpio_pdads_field_t PDADS_f;
    };
    union
    {
        __IO uint32_t PDBSET;                                 //0x1D0	端口 PD位置位寄存器（PDBSET） （对应PD15-PD00  	0/1:保持/置位
        stc_gpio_pdbset_field_t PDBSET_f;
    };
    union
    {
        __IO uint32_t PDBCLR;                                 //0x1D4	端口 PD 位清零寄存器（PDBCLR）	（对应PD15-PD00	0/1:保持/保持
        stc_gpio_pdbclr_field_t PDBCLR_f;
    };
    union
    {
        __IO uint32_t PDBSETCLR;                              //0x1D8	端口 PD 位置位清零寄存器（PDBSETCLR）
        stc_gpio_pdbsetclr_field_t PDBSETCLR_f;
    };
    union
    {
        __IO uint32_t PDDR;                                   //0x1DC	端口 PD 驱动能力配置寄存器（PDDR）	（对应PD15-PD00  0/1:高/低 驱动能力
        stc_gpio_pddr_field_t PDDR_f;
    };
    union
    {
        __IO uint32_t PDPU;                                   //0x1E0	端口 PD 上拉使能配置寄存器（PDPU）	（对应PD15-PD00）0/1:禁止/使能上拉
        stc_gpio_pdpu_field_t PDPU_f;
    };
    union
    {
        __IO uint32_t PDPD;                                   //0x1E4	端口 PD 下拉使能配置寄存器（PDPD）	（对应PD15-PD00）0/1:禁止/使能下拉
        stc_gpio_pdpd_field_t PDPD_f;
    };
    uint8_t RESERVED111[4];
    union
    {
        __IO uint32_t PDOD;                                   //0x1EC	端口 PD 开漏输出配置寄存器（PDOD）	（对应PD15-PD00）0/1:推完/开漏输出
        stc_gpio_pdod_field_t PDOD_f;
    };
    union
    {
        __IO uint32_t PDHIE;                                  //0x1F0	端口 PD 高电平中断使能配置寄存器（PDHIE）（对应PD15-PD00）0/1:禁止/使能 高电平中断
        stc_gpio_pdhie_field_t PDHIE_f;
    };
    union
    {
        __IO uint32_t PDLIE;                                  //0x1F4 	端口 PD 低电平中断使能配置寄存器（PDLIE）（对应PD15-PD00）0/1:禁止/使能 低电平中断
        stc_gpio_pdlie_field_t PDLIE_f;
    };
    union
    {
        __IO uint32_t PDRIE;                                  //0x1F8		端口 PD 上升沿中断使能配置寄存器（PDRIE）(对应PD15-PD00）0/1:禁止/使能上升沿中断
        stc_gpio_pdrie_field_t PDRIE_f;
    };
    union
    {
        __IO uint32_t PDFIE;                                  //0x1FC		端口 PD 下降沿中断使能配置寄存器（PDFIE）(对应PD15-PD00）0/1:禁止/使能下降沿中断
        stc_gpio_pdfie_field_t PDFIE_f;
    };
    union
    {
        __IO uint32_t PA_STAT;				 //0x200		端口 PA 中断状态寄存器（PA_STAT） （对应PA15-PA00）0/1:无/有中断触发
        stc_gpio_pa_stat_field_t PA_STAT_f;
    };
    uint8_t RESERVED117[12];
    union
    {
        __IO uint32_t PA_ICLR;				//0x210		端口 PA 中断清除寄存器（PA_ICLR）		（对应PA15-PA00）0/1:清楚/保留中断标志
        stc_gpio_pa_iclr_field_t PA_ICLR_f;
    };
    uint8_t RESERVED118[44];
    union
    {
        __IO uint32_t PB_STAT;				//0x240		端口 PB 中断状态寄存器（PB_STAT） （对应PB15-PB00）0/1:无/有中断触发
        stc_gpio_pb_stat_field_t PB_STAT_f;
    };
    uint8_t RESERVED119[12];
    union
    {
        __IO uint32_t PB_ICLR;				//0x250		端口 PB 中断清除寄存器（PB_ICLR）		（对应PB15-PB00）0/1:清楚/保留中断标志
        stc_gpio_pb_iclr_field_t PB_ICLR_f;
    };
    uint8_t RESERVED120[44];
    union
    {
        __IO uint32_t PC_STAT;				//0x280		端口 PC 中断状态寄存器（PC_STAT） （对应PC15-PC00）0/1:无/有中断触发
        stc_gpio_pc_stat_field_t PC_STAT_f;
    };
    uint8_t RESERVED121[12];
    union
    {
        __IO uint32_t PC_ICLR;				//0x290		端口 PC 中断清除寄存器（PC_ICLR）		（对应PC15-PC00）0/1:清楚/保留中断标志
        stc_gpio_pc_iclr_field_t PC_ICLR_f;
    };
    uint8_t RESERVED122[44];
    union
    {
        __IO uint32_t PD_STAT;				//0x2C0		端口 PD 中断状态寄存器（PD_STAT） （对应PD15-PD00）0/1:无/有中断触发
        stc_gpio_pd_stat_field_t PD_STAT_f;
    };
    uint8_t RESERVED123[12];
    union
    {
        __IO uint32_t PD_ICLR;				//0x2D0		端口 PD 中断清除寄存器（PD_ICLR）		（对应PD15-PD00）0/1:清楚/保留中断标志
        stc_gpio_pd_iclr_field_t PD_ICLR_f;
    };
    uint8_t RESERVED124[44];
    union
    {
        __IO uint32_t CTRL0;				//0x300 GPIO_CTRL0 RW 端口辅助功能配置寄存器 0	(不存在)
        stc_gpio_ctrl0_field_t CTRL0_f;
    };
    union
    {
        __IO uint32_t CTRL1;				//0x304 GPIO_CTRL1 RW 端口辅助功能配置寄存器 1
        stc_gpio_ctrl1_field_t CTRL1_f;
    };
    union
    {
        __IO uint32_t CTRL2;				//0x304 GPIO_CTRL1 RW 端口辅助功能配置寄存器 1
        stc_gpio_ctrl2_field_t CTRL2_f;
    };
    union
    {
        __IO uint32_t TIMGS;				//0x30c GPIO_TIMGS RW 端口辅助功能定时器门控选择
        stc_gpio_timgs_field_t TIMGS_f;
    };
    union
    {
        __IO uint32_t TIMES;				//0x310 GPIO_TIMES RW 端口辅助功能定时器ETR选择
        stc_gpio_times_field_t TIMES_f;
    };
    union
    {
        __IO uint32_t TIMCPS;				//0x314 GPIO_TIMCPS RW 端口辅助功能定时器捕获输入选择
        stc_gpio_timcps_field_t TIMCPS_f;
    };
    union
    {
        __IO uint32_t PCAS;				//0x318 GPIO_PCAS RW 端口辅助功能PCA捕获选择
...
typedef struct
{
    union
    {
        __IO uint32_t CR;				//0x00 SPIx_CR RW SPIx 配置寄存器
        stc_spi_cr_field_t CR_f;
    };
    union
    {
        __IO uint32_t SSN;				//0x04 SPIx_SSN RW SPIx 片选配置寄存器
        stc_spi_ssn_field_t SSN_f;
    };
    union
    {
        __IO uint32_t STAT;				//0x08 SPIx_STAT RO SPIx 状态寄存器
        stc_spi_stat_field_t STAT_f;
    };
    union
    {
        __IO uint32_t DATA;			//0x0c SPIx_DATA RW SPIx 数据寄存器
        stc_spi_data_field_t DATA_f;
    };
    union
    {
        __IO uint32_t CR2;				//0x10 SPIx_CR2 RW SPIx 配置寄存器 2
        stc_spi_cr2_field_t CR2_f;
    };
    union
    {
        __IO uint32_t ICLR;				//0x14 SPIx_ICLR WO SPIx 中断清除寄存器
