...
{
    //配置为默认值,GPIO功能
    setBit((uint32_t)&M0P_GPIO->PAADS + enPort, enPin, FALSE);										//配置为数字pin
    *((uint32_t*)(((uint32_t)(&(M0P_GPIO->PA00_SEL)) + enPort) + (((uint32_t)enPin)<<2))) = GpioAf0;		//数字pin功能选择为普通IO
...
{
	Gpio_ClearIrq(enPort, enPin);									//added by sky bough.cn
